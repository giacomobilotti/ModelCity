...
Dataset-specific loaders live here, not in the modelcity package.  The package
only provides ``as_settlements``, metrics, and plots.

    python scripts/run_unified.py
    python scripts/run_unified.py --dataset yautepec
"""
...
def read_yautepec(path, **kwargs) -> "mc.Settlements":
...
def read_viabundus(path, assign_region: bool = True, **kwargs) -> "mc.Settlements":
...
DATASETS: Dict[str, Tuple[Callable[[], "mc.Settlements"], str]] = {
    "yautepec": (lambda: read_yautepec(DATA / "yautepec.csv"), "yautepec/unified"),
    "viabundus": (lambda: read_viabundus(DATA / "viabundus.csv"), "viabundus/unified"),
}
